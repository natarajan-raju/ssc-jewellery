...
import { Loader2, Trash2, Search, Mail, Phone, Key, ShieldCheck, Plus } from 'lucide-react'; // Added Plus icon
import { useToast } from '../../context/ToastContext';
import Modal from '../../components/Modal';
import AddCustomerModal from '../../components/AddCustomerModal'; // Import New Modal
...
export default function Customers() {
    ...
    // New: Handle Add Customer
    const handleAddCustomer = async (userData) => {
        // The service call is awaited here. If it fails, it throws to the Modal's catch block.
        // If it succeeds:
        const res = await adminService.createUser(userData);
        setUsers([...users, res.user]); // Update list immediately
        setIsAddModalOpen(false); // Close Modal
        toast.success("Customer added successfully");
    };

    // Existing Handlers
    const openDeleteModal = (user) => { /* ... same as before ... */ 
    ...
    const openResetModal = (user) => { /* ... same as before ... */
        setModalConfig({ isOpen: true, type: 'input', title: 'Reset Admin Password', message: `Enter a new password for ${user.name}.`, targetUser: user });
    };
        ...
        <div className="animate-fade-in relative z-0">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
                <div>
                    ...
                    <p className="text-gray-500 text-sm mt-1">Manage user access & security</p>
                </div>
                ...
                <div className="flex gap-2 w-full md:w-auto">
                    ...
                    <button 
                        ...
                        className="bg-primary hover:bg-primary-light text-accent font-bold px-4 rounded-xl shadow-lg shadow-primary/20 flex items-center gap-2 transition-all active:scale-95 whitespace-nowrap"
                    >
                ...
                <>
                    ...
                    <div className="grid grid-cols-1 gap-4 md:hidden">
                        {filteredUsers.map((user) => (
                            <div key={user.id} className={`p-5 rounded-xl shadow-sm border relative ${user.role === 'admin' ? 'bg-amber-50 border-accent/30' : 'bg-white border-gray-100'}`}>
                                <div className="flex items-start gap-4">
                                    <div className={`w-12 h-12 rounded-full flex items-center justify-center shrink-0 ${user.role === 'admin' ? 'bg-accent text-primary' : 'bg-primary/5 text-primary'}`}>
                                        {user.role === 'admin' ? <ShieldCheck size={20} /> : <span className="font-bold">{user.name.charAt(0)}</span>}
                                    </div>
                                    ...
                                    <tr key={user.id} className="hover:bg-gray-50/50 transition-colors">
                                        <td className="px-6 py-4">
                                            <div className="flex items-center gap-3">
                                                <div className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-xs ${user.role === 'admin' ? 'bg-accent text-primary' : 'bg-primary/10 text-primary'}`}>
                                                    {user.role === 'admin' ? <ShieldCheck size={14}/> : user.name.charAt(0)}
                                                </div>
                                        ...
                                        <td className="px-6 py-4">
                                            {user.role === 'admin' ? 
                                                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-primary text-accent">ADMIN</span> : 
                                                <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800">Customer</span>
                                            }
                                        </td>
